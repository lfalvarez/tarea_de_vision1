...
def input_fn(filename, image_shape, mean_img, is_training, configuration):     
    ...
    dataset = dataset.batch(configuration.get_batch_size())   
    return dataset
#%%
#define the input function for prediction
def input_fn_for_prediction(the_image, image_shape, mean_img, number_of_channels, process_function):
    """
    ...
    if isinstance(the_image, str) : 
        image = read_image(the_image, number_of_channels)    
    else :
        image = load_image(the_image, number_of_channels)        
...
#%%
#define the input function for prediction
def input_fn_for_prediction_on_list(list_of_images, image_shape, mean_img, number_of_channels, process_function):    
...
#%% 
def load_image(np_image, number_of_channels):
...
"""    
def create_tfrecords_from_file(filenames, labels, target_shape, process_function, tfr_filename):
    h = target_shape[0]
    w = target_shape[1]
...
def create_tfrecords(str_path, id_type, im_size, number_of_channels, processFun):    
    """ 
    ...
    if ( id_type + 1 ) & 1 : # train   ( 0 + 1 ) & 1  == 1 
        filenames, labels = read_data_from_file(str_path, dataset="train", shuf = True)    
        tfr_filename = os.path.join(str_path, "train.tfrecords")
        training_mean = create_tfrecords_from_file(filenames, labels, image_shape, processFun, tfr_filename)
    ...
    if ( id_type + 1 ) & 2 : # test ( 1 + 1 ) & 2  == 2
        filenames, labels = read_data_from_file(str_path, dataset="test", shuf = True)  
        tfr_filename = os.path.join(str_path, "test.tfrecords")
        create_tfrecords_from_file(filenames, labels, image_shape, processFun, tfr_filename)
