...
def process_mnist_image(image, imsize):    
    """
    imsize = (h,w)
    """
    #resize uses format (w,h)
...
def get_process_fun(str_name = 'default') :
    if str_name == 'default' :
        return process_image
    else :
        raise ValueError("{} is not a valid processFun name".format(str_name))

def normalize_sketch(mat_sketch, im_size, obj_size, _erosion = True ):
...
def read_sketch(filename, im_size, obj_size, for_data = False):
    """
    ...
    if for_data :
        erosion = False
    im_query = normalize_sketch(im_query, im_size, obj_size, erosion)
    return im_query

def read_sketch_data(filename):
    """
    filename is the path to the image
    im_size is the size of the target image
    obj_size is the size of the maximum size of the sketch (foreground)
